...
Well, ok, assuming that $\langle x - z, w - z \rangle$ is purely real, then as we vary $t$, since the inner products are constants, this must hold for all $t \in (0,1]$, and hence, we have that:

$$ 2 \langle x - z, w - z \rangle \leq 0$$

as desired. I'm not sure why that inner product is purely real.
