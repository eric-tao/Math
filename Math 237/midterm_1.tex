...
Let $x, y \in \mathcal{H}$. Then, we have that, for $P$ the projection onto a closed, convex set $K$:

$$ \Vert P(x) - P(y) \Vert \leq \Vert x - y \Vert $$

\end{lemma}

\begin{proof}

First, suppose $P(x) = P(y)$. Then, for any $x, y$ this is true, due to the non-negativity of the norm.

Let $x, y$ be given. From the statement 4.1.ii, we have the following equations:

$$  \begin{cases} \langle x - P(x), P(y) - P(x) \rangle \leq 0 \\ \langle y - P(y), P(x) - P(y) \rangle \leq 0 \end{cases}$$

where in the first equation, we let $P(y) \in K$ play the role of $y$ in 4.1.ii, and $P(x)$ be $y$.

Now, summing these two equations, we have that:

$$ \langle -x + P(x), P(x) - P(y) \rangle + \langle y - P(y), P(x) - P(y) \rangle \leq 0 \implies \langle (-x + y) + (P(x) - P(y)), P(x) - P(y) \rangle \leq 0 $$

Hence, looking at $\Vert P(y) - P(x) \Vert^2$, we have that:

$$ \Vert P(x) - P(y) \Vert^2  = \langle P(x) - P(y), P(x) - P(y) \rangle \leq \langle P(x) - P(y),P(x) -  P(y) \rangle - \langle (-x + y) + (P(x) - P(y)), P(x) - P(y) \rangle$$

as the second quantity is at most 0, so subtracting it off creates a larger quantity. Now, combining these inner products, and pulling out two factors of $-1$ from the first term, we have that this is equal to:

$$ \langle P(x) - P(y) + x - y - P(x) + P(y), P(x) - P(y) \rangle = \langle x - y, P(x) - P(y) \rangle$$

By Cauchy-Schwarz then, we have that:

$$ \langle x - y, P(x) - P(y) \rangle \leq \Vert x - y \Vert \Vert P(x) - P(y) \Vert $$

where we do not worry about the absolute value, as this must be non-negative, being larger than a norm.

Hence, we have that:

$$  \Vert P(x) - P(y) \Vert^2  \leq \Vert x - y \Vert \Vert P(x) - P(y) \Vert \implies \Vert x - y \Vert$$
\end{proof}

Now, we apply the previous lemma on the vectors $\rho f - \rho Tv_1 + v_1, \rho f - \rho Tv_2 + v_2$, and their projections. We have that:

$$ \Vert S_\rho(v_1) - S_\rho(v_2) \Vert = \Vert P(\rho f - \rho Tv_1 + v_1) - P(\rho f - \rho Tv_2 + v_2) \Vert \leq $$

$$ \Vert \rho f - \rho Tv_1 + v_1 - \rho f + \rho Tv_2 - v_2 \Vert  = \Vert (v_1  - v_2) - \rho T(v_1 - v_2) \Vert $$
...
Looking at that coefficient, we see that to have $ 1 + \rho^2 \Vert T\Vert^2 - 2 \rho \alpha < 1$, this implies that, since this is a parabola in $\rho$ opening upwards:

$$ \rho^2 T^2 - 2 \rho \alpha < 0 \implies \rho(\rho T^2 - 2 \alpha) < 0 \implies \rho \in \left( 0, \frac{ 2\alpha}{\Vert T\Vert^2}\right)$$
...
for some $k \in (0,1)$. Hence, taking the square root of both sides, for this choice of $\rho$, $k$ remains a bound.
